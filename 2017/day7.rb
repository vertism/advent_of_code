...
class Day7
  ...
  def run
    ...
    with_parent = []
    programs = []
    INPUT.each_line do |program_data|
      program_string, above_string = program_data.split('->')
      name, weight = program_string.split
      programs << name
      if above_string
        with_children << name
        above = above_string.split(', ')
        with_parent << above.map(&:strip)
      end
    end

    with_children.uniq - with_parent.flatten.uniq
  end
end
