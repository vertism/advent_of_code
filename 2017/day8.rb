...
class Day8
  ...
  def run
    ...
    INPUT.each_line do |instruction|
      tokens = parse(instruction)
      registers[tokens[:register]] ||= 0
      registers[tokens[:register2]] ||= 0
      if registers[tokens[:register2]].send(tokens[:comparison], tokens[:value2])
        if tokens[:command] == 'inc'
          registers[tokens[:register]] += tokens[:value]
        else
          registers[tokens[:register]] -= tokens[:value]
        end
      end
