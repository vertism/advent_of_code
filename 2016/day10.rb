...
class Day10
  ...
  def initialize
    @input = "bot 171 gives low to bot 4 and high to bot 84
  ...
  def run
    ...
    while true
      @input.split("\n").map(&:strip).each do |line|
        if line =~ /^bot/
          if (result = transfer(line)) != 0
            part1 = result
          end
        end
      end
      if outputs['0'].size > 0 && outputs['1'].size > 0 && outputs['2'].size > 0 && part1 != '0'
        break
      end
    end
    
    puts "Part 1: #{part1}"
    puts "Part 2: #{outputs['0'].first * outputs['1'].first * outputs['2'].first}"
  end
  
  def give(line)
    if match = /(\d+)[^\d]+(\d+)/.match(line)
      @bots[match[2]] << match[1].to_i
    end
  end
  
  def transfer(line)
    if match = /bot (\d+) gives low to (\w+) (\d+) and high to (\w+) (\d+)/.match(line)
      bot = match[1]
      ...
      low = @bots[bot].sort.first
      high = @bots[bot].sort.last
      
      collection_1 = send("#{match[2]}s")
      collection_1[match[3]] << low
      
      collection_2 = send("#{match[4]}s")
      collection_2[match[5]] << high
