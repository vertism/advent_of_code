...
class Day4
  ...
  def run
    rooms = @inputs.map { |data| Room.new(data) }
    real_rooms = rooms.select(&:real?)
    
    total = real_rooms.map(&:sector_id).inject(&:+)
    puts "Part 1: #{total}"
    
    north_pole_room = real_rooms.find { |r| r.sector_id if r.name.include?('north') }
    puts "Part 2: #{north_pole_room.sector_id}"
  end
end
...
class Room
  ...
  def name
    @name ||= encrypted_name.chars.map do |letter|
      if letter == '-'
        ' '
      else
        start = letter.ord - 97
        moved = (start + sector_id) %  26
        (moved + 97).chr
      end
    end.join('')
  end
  ...
  def top_chars
    letters = Hash.new(0)
    encrypted_name.gsub(/-/,'').chars.each do |letter|
      letters[letter] += 1
    end
    letters
      .to_a
      .sort{ |x,y| [y[1], x[0]] <=> [x[1], y[0]] }
      .take(5)
      .map{ |l| l[0] }
      .join('')
  end
