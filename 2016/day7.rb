  ...
  def run
    tsl = @inputs.select do |input|
      ...
      input.chars.each_with_index do |char, index|
        ...
        if (char == input[index-3] && input[index-2] == input[index-1] && char != input[index-1])
    ...
    puts "Part 1: #{tsl.size}"
    
    ssl = @inputs.select do |input|
      in_square_brackets = false
      valid = false
      input.chars.each_with_index do |char, index|
        if char == "["
        ...
        if (char == input[index-2] && char != input[index-1])
          bab = "#{input[index-1]}#{char}#{input[index-1]}"
          if input =~ /\[[a-z]*#{bab}[a-z]*\]/
            valid = true
            break
          end
        end
